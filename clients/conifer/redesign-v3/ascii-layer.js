...
(function (global) {
  var STORAGE_KEY = "conifer-ascii-pure-v1";

  /* v3 — the bonsai is drawn 100% in glyphs, no photo underneath */
  var DEFAULTS = {
    cellSize: 6,
    cellRatio: 1.6,
    opacity: 1,
  ...
  AsciiLayer.prototype._buildGrid = function (hostW, hostH, cols, rows) {
    ...
    if (c.normalize) this._normalizeGrid(grid);
    this.grid = { cols: cols, rows: rows, data: grid, w: hostW, h: hostH, key: this._gridKey() };
  };

  /* Stretch the sampled darkness so the ground (20th pct) → 0 and the darkest
     ink (99.7th pct) → 1. The source photo is light-on-light, so without this a
     pure-glyph render stays in the palest tones. */
  AsciiLayer.prototype._normalizeGrid = function (grid) {
    var hist = new Uint32Array(256);
    var n = grid.length;
    for (var i = 0; i < n; i++) hist[Math.min(255, (grid[i] * 255) | 0)]++;
    var lo = 0, hi = 255, acc = 0;
    for (var b = 0; b < 256; b++) { acc += hist[b]; if (acc >= n * 0.2) { lo = b; break; } }
    acc = 0;
    for (var b2 = 255; b2 >= 0; b2--) { acc += hist[b2]; if (acc >= n * 0.003) { hi = b2; break; } }
    var loV = lo / 255, hiV = Math.max(hi / 255, loV + 0.02), span = hiV - loV;
    var gamma = this.localConfig.gamma || 1;
    for (var k = 0; k < n; k++) {
      var v = (grid[k] - loV) / span;
      v = v < 0 ? 0 : v > 1 ? 1 : v;
      grid[k] = gamma === 1 ? v : Math.pow(v, gamma);
    }
  };

  AsciiLayer.prototype._gridKey = function () {
    var c = this.localConfig;
    return [c.imgScaleX, c.imgScaleY, c.imgOffsetX, c.imgOffsetY, c.normalize, c.gamma].join("|");
  };
  ...
  AsciiLayer.prototype.render = function () {
    ...
    ctx.font = (c.fontWeight || 400) + " " + fontSize + "px " + c.fontFamily;
