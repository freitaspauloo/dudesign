...
const h = fs.readFileSync("clients/conifer/site/index.html", "utf8");
const svgs = [...h.matchAll(/<svg[\s\S]*?<\/svg>/g)].map((m) => m[0]);
fs.mkdirSync("clients/conifer/assets", { recursive: true });
...
const best = ranked[0];
fs.writeFileSync("clients/conifer/assets/conifer-mark.svg", best.s);
